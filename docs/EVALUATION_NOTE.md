...
The fundamental requirement of the project is that a user (or automated system) can upload an e-commerce export file (e.g., Shopify Sales CSV), and the system will model it and push it into Tally without errors.

**Test Scenario:**
1. Upload a standard `Shopify_Sales.csv`.
2. Select Cutover Date and target Tally Company.
3. System bypasses manual mapping (uses Shopify template).
...
- **Fractional Rounding (Zero-Sum):** If a Shopify order calculates CGST/SGST resulting in a 0.01 mismatch against the Gross Total, the pipeline must NOT fail. It must inject a `Round Off` ledger entry to balance the voucher exactly to 0.00.
...
- [ ] Implement Shopify Mapping Template logic.
